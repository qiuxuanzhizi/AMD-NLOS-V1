function [X_den,iter,fun_all]=denoise_bound_3D(Xobs,lambda,l,u,pars)
...
while((i<MAXITER)&(count<5))
    ...
    Pold=P;
    tk=tkp1;
    D=project(Xobs-lambda*Lforward_3D(R));
    Q=Ltrans_3D(D);
    ...
    P{3}=R{3}+1/(8*lambda)*Q{3};
    %%%%%%%%%%
    % Peforming the projection step
    switch tv
        case 'iso'
            A=[P{1};zeros(1,n,p)].^2+[P{2},zeros(m,1,p)].^2+[P{3},zeros(m,n,1)].^2;
            A=sqrt(max(A,1));
            P{1}=P{1}./A(1:m-1,:,:);
            P{2}=P{2}./A(:,1:n-1,:);
            P{3}=P{3}./A(:,:,1:p-1);
    ...
    re=normfro_3D(D-Dold)/normfro_3D(D);
    if (re<epsilon)
        count=count+1;
    else
        count=0;
    ...
    PC=project(C);
    fval=-normfro_3D(C-PC)^2+normfro_3D(C)^2;
